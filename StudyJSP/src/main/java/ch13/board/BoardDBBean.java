...
public class BoardDBBean {
	...
	// .jsp 페이지에서 DB 연동빈인 BoardDBBean 클래스의 메소드에 접근시 필
	...
	// board 테이블에 저장된 전체 글의 수를 얻어냄.(select문) <= list.jsp 에서 사용
	...
	// 글의 목록(복수 개의 글)을 가져옴 (select문) <= list.jsp 에서 사용
	...
	// 글 수정 폼에서 사용할 글의 내용(1개의 글) (select문) <= updateForm.jsp 에서 사용
	...
	// 글 삭제 처리시 사용(delete문) <= deletePro.jsp 페이지에서 사용
	@SuppressWarnings("resource")
